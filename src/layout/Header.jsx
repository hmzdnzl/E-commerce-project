...
import { Phone, Mail, User, Search, ShoppingCart, Heart, Menu, ChevronDown, Trash } from "lucide-react";
...
export default function Header() {
 ...
 /*  console.log("Tüm kategoriler:", categories.map(c => c.title));
  console.log("Kadın kategorileri:", womenCategories.map(c => c.title));
  console.log("Erkek kategorileri:", menCategories.map(c => c.title)); */
...
console.log(JSON.stringify(userState) + "aa");
        ...
        <div className="relative group inline-block">
          ...
          <div className="absolute left-0 top-full w-80 bg-white shadow-lg rounded z-50 flex flex-row gap-6 p-4
            ...
            <div className="flex flex-col min-w-[100px]">
              ...
              {womenCategories.length === 0 && <span className="px-2 text-sm text-gray-400">Kategori yok</span>}
              {womenCategories.map(cat => (
                ...
                >
                  {cat.img && <img src={cat.img} alt={cat.title} className="w-6 h-6" />}
                  {cat.title}
                </a>
              ))}
            </div>
            <div className="flex flex-col min-w-[100px]">
              ...
              {menCategories.length === 0 && <span className="px-2 text-sm text-gray-400">Kategori yok</span>}
  ...
  <div className="absolute right-0 top-full w-80 bg-white shadow-lg rounded z-50 flex flex-row gap-6 p-4
    ...
    <div className="flex flex-col m-auto w-auto px-3 gap-1 h-auto">
      <span className="font-bold text-[#252B42] text-center mb-2">Shopping Cart</span>
      {cart.length === 0 && <span className="px-2 text-sm text-gray-400">Sepetiniz boş</span>}
      <section>
        <div className="flex flex-col gap-5">
          {cart.map((item) => (
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => increaseQuantity(item)} className="border border-black w-6 h-6 flex items-center justify-center font-bold text-center rounded text-xs leading-none p-0">+</button>
              <p>{item.quantity}</p>
              <button onClick={()=> item.quantity>1? decreaseQuantity(item):item.quantity} className="border border-black w-6 h-6 flex items-center justify-center font-bold text-center rounded text-xs leading-none p-0">-</button>
